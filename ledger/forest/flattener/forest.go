...
func RebuildTries(db *badger.DB, flatForest *FlattenedForest) ([]*trie.MTrie, error) {
	tries := make([]*trie.MTrie, 0, len(flatForest.Tries))
	nodes, err := RebuildNodes(db, flatForest.Nodes)
	if err != nil {
...
func RebuildNodes(db *badger.DB, storableNodes []*StorableNode) ([]*node.Node, error) {
	...
	for i, snode := range storableNodes {
		...
		if len(snode.Path) > 0 {
			...
			node := node.NewNode(db, int(snode.Height), nodes[snode.LIndex], nodes[snode.RIndex], path, nodeHash, snode.MaxDepth, snode.RegCount)
		...
		node := node.NewNode(db, int(snode.Height), nodes[snode.LIndex], nodes[snode.RIndex], ledger.DummyPath, nodeHash, snode.MaxDepth, snode.RegCount)
		nodes = append(nodes, node)
	}
	return nodes, nil
}
