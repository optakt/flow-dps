...
// TODO: This is the original file, as is. Go through this file and check what can be changed/improved.
...
func EncodeStorableNode(storableNode *StorableNode) []byte {
	...
	length := 2 + 2 + 8 + 8 + 2 + 8 + 2 + len(storableNode.Path) + 2 + len(storableNode.HashValue)
...
func ReadStorableNode(reader io.Reader) (*StorableNode, error) {
	...
	// FIXME: If needed, get the payload from the db.
