...
import (
	...
	flattener2 "github.com/optakt/flow-dps/ledger/forest/flattener"
)

func TestStorableNode(t *testing.T) {
	path := utils.PathByUint8(3)

	storableNode := &flattener2.StorableNode{
		LIndex:     1,
		RIndex:     2,
		Height:     2137,
		Path:       path[:],
	...
	t.Run("encode", func(t *testing.T) {
		data := flattener2.EncodeStorableNode(storableNode)
		assert.Equal(t, expected, data)
	})

	t.Run("decode", func(t *testing.T) {
		reader := bytes.NewReader(expected)
		newStorableNode, err := flattener2.ReadStorableNode(reader)
...
func TestStorableTrie(t *testing.T) {
	...
	storableTrie := &flattener2.StorableTrie{
		RootIndex: 21,
		RootHash:  []byte{2, 2, 2},
	}
	...
	t.Run("encode", func(t *testing.T) {
		data := flattener2.EncodeStorableTrie(storableTrie)
	...
	t.Run("decode", func(t *testing.T) {
		...
		newStorableNode, err := flattener2.ReadStorableTrie(reader)
		require.NoError(t, err)
