...
func (c *Checkpointer) Checkpoint(db *badger.DB, to int, targetWriter func() (io.WriteCloser, error)) error {
	...
	forest, err := forest.NewForest(c.forestCapacity, &metrics.NoopCollector{}, func(evictedTrie *trie.MTrie) error {
		return nil
	})
	if err != nil {
	...
	err = c.wal.replay(0, to,
		func(forestSequencing *flattener.FlattenedForest) error {
			tries, err := flattener.RebuildTries(db, forestSequencing)
			if err != nil {
				return err
			}
			for _, t := range tries {
