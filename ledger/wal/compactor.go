...
func (c *Compactor) Ready(db *badger.DB) <-chan struct{} {
	c.lm.OnStart(func() {
		go c.start(db)
	})
	return c.lm.Started()
...
func (c *Compactor) start(db *badger.DB) {
	for {
		//TODO Log error
		_ = c.Run(db)
...
func (c *Compactor) Run(db *badger.DB) error {
	c.Lock()
	defer c.Unlock()

	newLatestCheckpoint, err := c.createCheckpoints(db)
	if err != nil {
...
func (c *Compactor) createCheckpoints(db *badger.DB) (int, error) {
	...
	if to-from > int(c.checkpointDistance) {
		...
		err = c.checkpointer.Checkpoint(db, checkpointNumber, func() (io.WriteCloser, error) {
