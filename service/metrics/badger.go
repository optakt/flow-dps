...
func RegisterBadgerMetrics() error {
	expvarCol := prometheus.NewExpvarCollector(map[string]*prometheus.Desc{
		...
		"badger_pending_writes_total": prometheus.NewDesc("badger_badger_pending_writes_total", "tracks the number of pending writes", []string{"path"}, nil),
		"badger_lsm_bloom_hits_total": prometheus.NewDesc("badger_lsm_bloom_hits_total", "number of LSM bloom hits", []string{"level"}, nil),
		"badger_lsm_level_gets_total": prometheus.NewDesc("badger_lsm_level_gets_total", "number of LSM gets", []string{"level"}, nil),
